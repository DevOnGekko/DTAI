...
## Test-only Azure Secure Key Release harness

`tools/Dtai.Agent/Attestation` contains a **test-only** integration that runs
the real Azure leg of the flow: a TEE attestation via **Microsoft Azure
Attestation (MAA)** → **Azure Key Vault** Secure Key Release. It exercises the
`MaaAttestationService` (K1/MAA) and `KeyVaultKeyProvider` (Key Vault release)
providers; the ITA/Hashicorp K2 path stays a stub.

The TEE type is selected by `AttestationType` and drives both the MAA attest
path (`/attest/{type}`), the request shape, and the MAA API version. **AMD
SEV-SNP (`SevSnpVm`) and Intel TDX (`TdxVm`) are implemented** with bundled
sample evidence and identical MAA-token verification (signature via MAA's JWKS,
issuer, lifetime); `AzureGuest` is wired as an explicit dispatch point in
`TeeEvidenceProvider` and `MaaAttestationService` and throws
`NotSupportedException` until its evidence and request shape are added.

> **TDX sample caveat.** Unlike a SEV-SNP report, which carries its full AMD
> VCEK→ASK→ARK chain in-band, a TDX quote relies on Intel PCS collateral that
> MAA fetches out-of-band and that must be current. The bundled TDX sample is a
> documentation quote whose collateral is stale, so live MAA rejects it with
> `InvalidQuote` ("error in the quote collateral"). The request shaping and
> token verification are identical to SEV-SNP and correct — supply a fresh quote
> from a real TDX CVM to obtain a token. (SEV-SNP works end-to-end as shipped.)

It is off by default. When no vault and key are configured, every provider
returns a demo marker and makes **no** network calls (the default demo flow is
unchanged). It turns on only when a vault and key are supplied.

### Configure (no secrets committed)

Committed defaults in `Attestation/attestation.settings.json` contain no
subscription, tenant, or vault names. Supply real values for testing in either:

- `Attestation/attestation.settings.local.json` (git-ignored), e.g.
  ```json
  { "KeyVaultName": "your-premium-vault", "KeyName": "your-exportable-key" }
  ```
- or environment variables: `DTAI_ATTESTATION_KEYVAULTNAME`,
  `DTAI_ATTESTATION_KEYNAME`, `DTAI_ATTESTATION_TYPE`,
  `DTAI_ATTESTATION_TENANTID`, `DTAI_ATTESTATION_KEYVERSION`,
  `DTAI_ATTESTATION_MAAENDPOINT`.

`AttestationType` (or `DTAI_ATTESTATION_TYPE`) selects the TEE type and defaults
to `TdxVm`; set it to `SevSnpVm` for the self-contained path that attests
end-to-end against the shared MAA endpoint without provisioning TDX collateral.
The MAA API version follows the type automatically (SEV-SNP
`2022-08-01`, TDX `2025-06-01`) unless `AttestationApiVersion` is set. The Key
Vault key must be an HSM-backed, **exportable** key with a release
policy (Premium vault). Authentication uses `DefaultAzureCredential` (reuses an
existing sign-in or prompts interactively); no subscription ID is needed for the
data-plane release call.

### Run and verify

With a vault/key configured, the decrypt path runs the real leg and logs each
verification step:

```
Submit the evidence to MAA to get a token
  [maa] attesting SevSnpVm at https://sharedwus.wus.attest.azure.net/attest/SevSnpVm?api-version=2022-08-01
  [maa] fetching signing keys (JWKS): https://sharedwus.wus.attest.azure.net/certs
  [maa] token verified: signature OK, issuer https://sharedwus.wus.attest.azure.net, expires ...
  [maa] x-ms-attestation-type: sevsnpvm
Submit the request for K1 to Azure KeyVault
  [kv] releasing key '<key>' from <vault> (MAA token as release target)
  [kv] release authorized by MAA token; received wrapped key (<n>-char JWE)
```

The MAA token is verified locally (signature against MAA's `/certs` JWKS,
issuer, lifetime) before use; Key Vault re-verifies it authoritatively and only
releases the key when its claims satisfy the key's release policy. For testing,
the release policy should authorize the attestation types you exercise, e.g.
`anyOf` over `x-ms-attestation-type` `sevsnpvm` and `tdxvm` from the same MAA
authority.

Unit tests cover the safety gate and demo defaults (no network):

```powershell
dotnet test tools/Dtai.Agent.Tests
```

### Bundled evidence — safe to publish

`Attestation/Evidence/snp-report.b64url`, `vcek-cert-chain.b64url`,
`tdx-quote.b64url`, and `tdx-runtime-data.b64url` are Azure's **public**
`AttestSevSnpVm` / `AttestTdxVm` samples: an AMD SEV-SNP report plus its AMD
VCEK→ASK→ARK **public** certificate chain, and an Intel TDX quote plus its
runtime data. They contain no private keys and no secrets — attestation evidence
is meant to be shared with a verifier — and they belong to Microsoft's sample
hosts, not to any deployment, so there is no PII. (Do not commit evidence
captured from your own production CVM: a VCEK / TDX quote encodes that host's
chip identity.)
